...
# כתיבת גרסת HTML מעוצבת של הדוח לצד ההדפסה הרגילה ל-stdout
python -m agent.cli report --html report.html
...
**פלט לדוגמה:** הדוח כולו בעברית, כולל סעיף הניתוח החכם; בקשות איטיות
מקובצות לפי פעולה (method + path מנורמל, ללא query string/מזהי Mongo)
במקום שורה נפרדת לכל בקשה.

```
סיכום פעילות לוח המכרזים
תקופה: 2026-01-01 עד 2026-12-31
----------------------------------------
יצירה       :      7
רישום       :     12
עריכה       :      4
...
צפייה       :    421
----------------------------------------
אחר         :      7   (שורות לוג שאינן משויכות לפעולת מכרז)
לא תקין     :      0   (רשומות לוג פגומות או לא קריאות)
----------------------------------------
סה"כ        :    451
----------------------------------------
חריגות
  זוהו 2 מקרי הגשה כפולה
    משתמש=... ארגון=... מכרזים=[...] (הפרש של 0.603 שניות)
  56 בקשות חרגו מסף זמן התגובה
    POST /tender-board/:id/apply: ממוצע 6907ms (3 בקשות, מקסימום 8747ms)
    GET /tender-board/smart-search: ממוצע 3105ms (32 בקשות, מקסימום 6834ms)
    POST /tender-board/smart-create: ממוצע 2850ms (14 בקשות, מקסימום 3730ms)
----------------------------------------
ניתוח בינה מלאכותית
  רמת הפעילות נראית סבירה, עם מספר צפיות גבוה המעיד על מעורבות משתמשים.
  דפוס שגיאה: שגיאות חוזרות בהגשת מכרז
  חריגה: זוהתה הגשה כפולה של מכרז
  רמת ביטחון: 0.95
```

קוד יציאה `0` = הצלחה, `1` = שגיאת חיבור/שליפה (למשל URI שגוי) — הדו"ח
עדיין יודפס, עם הודעת `שגיאה` במקום הספירות.

**`--html PATH`:** כותב, בנוסף להדפסה הרגילה, גרסת HTML עצמאית ומעוצבת
(RTL, CSS מוטבע) של אותו דוח בדיוק לנתיב שצוין - הוכן כהכנה לאינטגרציה
עתידית (טרם מתוכננת בפירוט) עם `client/`. כל טקסט חופשי (ניתוח ה-AI,
הודעות שגיאה) עובר `html.escape()` לפני הטמעה ב-HTML.

**סעיף "ניתוח בינה מלאכותית" (SCRUM-180):** אחרי `errors`, הדוח מריץ
קריאת LLM אחת (`ChatOpenAI`) שמקבלת את הסיכום המספרי + מדגם קטן ומסונן
של תוכן עסקי אמיתי (כותרות מכרזים, הודעות שגיאה — לאחר סינון
prompt-injection ע"י `guardrail_node`), ומחזירה בעברית הערכת הגיון עסקי,
דפוסי שגיאה, ואנומליות בשפה טבעית. `evaluator_node` בודק שהתשובה
עקבית מול הנתונים בפועל (לא "ממציאה" כפילות/שגיאות שלא קיימות - הבדיקה
מזהה ביטויים קנוניים בעברית כמו "כפול"/"כפיל" ו"שגיאות"+"חוזרות"/"עלייה"
וכו', וגם את המקבילות באנגלית כרשת ביטחון למקרה שהמודל מתעלם מההנחיה
לכתוב בעברית) ולא מאשר תשובה עם ביטחון (`confidence`) נמוך מ-0.5;
במקרה כשל — עד 3 ניסיונות חוזרים ל-`analyze_node`, ואם גם אז לא אושר,
הסעיף מציין שהניתוח לא זמין באותה הרצה (הספירות/שגיאות/אנומליות עדיין
מוצגות במלואן — רק הניתוח החכם חסר).
...
│   ├── report.py           # (SCRUM-39/166/180) פורמט הדו"ח בעברית + שגיאות/חריגות/ניתוח AI + קיבוץ latency לפי endpoint
│   ├── html_report.py       # גרסת HTML עצמאית (RTL, escape) של אותו דוח - לשימוש עם --html
